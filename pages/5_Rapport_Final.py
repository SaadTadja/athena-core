"""
Athena Core ó Page 5: Rapport Final ExÈcutif
"""
...
st.set_page_config(page_title="Rapport Final ó Athena Core", layout="wide")
inject_css()
...
with col_title:
    page_header("Rapport ExÈcutif StratÈgique", f"GÈnÈrÈ automatiquement le {datetime.now().strftime('%d %B %Y')}")
with col_btn:
    st.markdown("<br>", unsafe_allow_html=True)
    if st.button("üñ®Ô∏è Imprimer / Sauvegarder en PDF", type="primary"):
        st.markdown("<script>window.print()</script>", unsafe_allow_html=True)

st.markdown("---")

# 1. Chargement des donnÈes globales
oi = load_order_items()
orders = load_orders()
...
# 2. Executive Summary
st.markdown("### 1. RÈsumÈ Financier")
...
try:
    ...
    st.markdown(f"""
    L'analyse de la matrice BCG rÈvËle que le portefeuille contient **{int(stars)} produit(s) Star** (fort potentiel de croissance et forte part de marchÈ) 
    qui nÈcessitent des investissements soutenus pour maintenir leur position dominante. En revanche, **{int(dogs)} produit(s) Dog** ont ÈtÈ identifiÈs, nÈcessitant une rÈvision stratÈgique urgente (optimisation des co˚ts ou retrait du marchÈ).
    """)
except Exception as e:
    st.warning("DonnÈes insuffisantes pour l'analyse stratÈgique.")

st.markdown("<br>", unsafe_allow_html=True)

# 4. Plan d'Action IA
st.markdown("### 3. Plan d'Action RecommandÈ (GÈnÈrÈ par l'IA)")
try:
    ...
    if high_priority:
        for rec in high_priority:
            ...
            <div style="border-left: 4px solid #EF4444; padding-left: 15px; margin-bottom: 10px;">
                <strong>PRIORIT… HAUTE : {rec['action']}</strong> ó {rec['target']}<br>
                <em>Justification :</em> {rec['reason']}
            </div>
            """, unsafe_allow_html=True)
    else:
        st.info("Aucune recommandation critique immÈdiate. Le systËme est stable.")
        
except Exception as e:
    st.error("Erreur lors de la gÈnÈration des recommandations.")

st.markdown("<br>", unsafe_allow_html=True)

# 5. Export Markdown
st.markdown("### Exportation des DonnÈes Brutes")
markdown_report = f"""# Rapport ExÈcutif Athena Core
GÈnÈrÈ le: {datetime.now().strftime('%Y-%m-%d')}
...
st.download_button(
    label="TÈlÈcharger le rapport brut (TXT/Markdown)",
    data=buffer,
    file_name="rapport_executif.txt",
    mime="text/plain"
)

st.caption("Athena Core ó Plateforme d'Intelligence DÈcisionnelle Confidentielle.")
